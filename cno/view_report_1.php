...
require '../otp/mailer.php'; // ✅ Added PHPMailer include for email notifications
    ...
    if (in_array($action, ['Approved', 'Rejected'])) {
        ...
        if (isset($_SESSION['user_id'])) {
            ...
            if ($reportOwnerId) {
                ...
                $notifLink = "reports.php?id=" . $reportId;
...
.document{background:#fff;width:21cm;min-height:33cm;margin:0 auto 30px auto;padding:2.5cm;box-shadow:0 0 8px rgba(0,0,0,0.15);position:relative;}
...
.header-logos img{max-height:60px;width:auto;display:inline-block;}
...
.action-buttons a, .action-buttons button {
    background:#6c757d;color:#fff;padding:6px 12px;border-radius:4px;
    text-decoration:none;border:none;cursor:pointer;margin-left:5px;
}
.action-buttons .approve {background:#28a745;}
.action-buttons .decline {background:#dc3545;}
</style>
</head>
<body>
<div class="layout">
...
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:15px;">
    ...
    <div class="action-buttons">
        ...
        <form method="post" style="display:inline;">
            <input type="hidden" name="report_id" value="<?= $row['reports_id'] ?>">
            <button type="submit" name="action" value="Rejected" class="decline"><i class="fa fa-times"></i> Decline</button>
        </form>
        <form method="post" style="display:inline;">
            <input type="hidden" name="report_id" value="<?= $row['reports_id'] ?>">
            <button type="submit" name="action" value="Approved" class="approve"><i class="fa fa-check"></i> Approve</button>
        </form>
    </div>
</div>
